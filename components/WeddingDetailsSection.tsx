...
/* Icons for the programme timeline — order matches t.details.events */
const EVENT_ICONS = [
  // Photoshoot — camera
  <svg key="0" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>,
  // Cocktail — guests
  <svg key="1" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
  // Dinner — utensils
  <svg key="2" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>,
  // Ends — clock
  <svg key="3" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
];

/* Icons for the morning programme — order matches t.details.morningEvents */
const MORNING_ICONS = [
  // Marriage / 嫁娶 — heart
  <svg key="0" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
  // Tea ceremony — teacup
  <svg key="1" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 9h2a3 3 0 0 1 0 6h-2"/><path d="M4 9h13v5a5 5 0 0 1-5 5H9a5 5 0 0 1-5-5V9z"/><path d="M7 2c0 1-1 1-1 2s1 1 1 2M11 2c0 1-1 1-1 2s1 1 1 2"/></svg>,
];
...
/* A single programme timeline (heading + vertical event list). */
function ProgrammeBlock({
  ...
  return (
    <div className="reveal-section space-y-0">
      <p className="font-inter text-[10px] tracking-[0.3em] uppercase mb-6" style={{ color: "rgba(255,255,255,0.35)" }}>{title}</p>
      {events.map((event, i) => (
        <div key={event.title} className="flex gap-4">
          ...
          <div className="pb-6">
            ...
            {event.location && (
              <p className="font-inter text-[11px] mt-1 flex items-center gap-1" style={{ color: "rgba(245,196,216,0.75)" }}>
                <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="#e090b0" strokeWidth="1.6" className="flex-shrink-0"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                {event.location}
              </p>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md mx-auto px-8 py-20">
        ...
        {/* Morning programme — tea ceremonies */}
        <ProgrammeBlock
          title={t.details.morningProgramme}
          events={t.details.morningEvents}
          icons={MORNING_ICONS}
        />

        {/* Divider between the two programmes */}
        <div className="reveal-section flex items-center gap-3 my-8">
          <div className="flex-1 h-px" style={{ background: "rgba(224,144,176,0.2)" }} />
          <svg viewBox="0 0 16 16" width="8" height="8" fill="#e090b0" opacity="0.5"><polygon points="8,0 16,8 8,16 0,8" /></svg>
          <div className="flex-1 h-px" style={{ background: "rgba(224,144,176,0.2)" }} />
        </div>
        ...
        {/* Dinner programme */}
        <ProgrammeBlock
          title={t.details.dinnerProgramme}
          events={t.details.events}
          icons={EVENT_ICONS}
        />
      </div>
    </section>
